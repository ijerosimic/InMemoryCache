...
public class InMemoryCacheTests
{
    ...
    {
        var mockOptions = Options.Create(new InMemoryCacheOptions { MaxItems = 5 });
        var sut = new InMemoryCache<object>(mockOptions);

        sut.Set(_kvp1.key, _kvp1.value, out _);
        var get = sut.Get(_kvp1.key);
        sut.Get(_kvp1.key).Should().Be(_kvp1.value);
        sut.GetCount().Should().Be(1);
        sut.GetThreshold().Should().Be(5);
    }
    ...
    [Fact]
    public void It_Returns_Null_When_Calling_Get_On_Empty_Cache()
    {
        var mockOptions = Options.Create(new InMemoryCacheOptions { MaxItems = 5 });
        var sut = new InMemoryCache<object>(mockOptions);

        sut.Get(_kvp1.key).Should().BeNull();
    }

    [Fact]
    public void It_Returns_Null_When_Calling_Get_With_Nonexistent_Key()
    {
        var mockOptions = Options.Create(new InMemoryCacheOptions { MaxItems = 5 });
        var sut = new InMemoryCache<object>(mockOptions);
        ...
        sut.Get("non-existent-key").Should().BeNull();
    }
    ...
    public void It_Evicts_The_Least_Recently_Used_Item_When_Setting_New_Item_Above_Threshold()
    {
        ...
        // Refresh positions
        sut.Get(_kvp1.key).Should().Be(_kvp1.value);
        sut.Get(_kvp2.key).Should().Be(_kvp2.value);

        // Set new item - trigger LRU eviction for "key-3"
        sut.Set(_kvp4.key, _kvp4.value, out _);

        sut.GetCount().Should().Be(3);
        sut.GetThreshold().Should().Be(3);

        // LRU is evicted
        sut.Get(_kvp3.key).Should().BeNull();

        sut.Get(_kvp1.key).Should().Be(_kvp1.value);
        sut.Get(_kvp2.key).Should().Be(_kvp2.value);
        sut.Get(_kvp4.key).Should().Be(_kvp4.value);
    }
}